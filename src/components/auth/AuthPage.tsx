...
export function AuthPage({ onSuccess, addToast }: Props) {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: 'https://content-and-editoria-h1nb.bolt.host/auth/callback',
        },
      });
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">ContentCal</h1>
          <p className="text-gray-500 text-sm mt-1">Your editorial calendar, organized.</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          <div className="flex rounded-lg bg-surface p-1 mb-6">
            ...
            <button
              ...
              className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${mode === 'signup' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Sign up
            </button>
          </div>
