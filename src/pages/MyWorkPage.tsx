...
export function MyWorkPage() {
  ...
  // Filter out completed items (items whose status is in a "done" column - last column by position)
  const activeItems = useMemo(() => {
    if (boardColumns.length === 0) return contentItems;
    const lastColumn = boardColumns[boardColumns.length - 1];
    return contentItems.filter((item) => item.status !== lastColumn.id);
                    ...
                    {activeItems.map((item) => {
                      ...
                      return (
                        <tr
                          ...
                          style={isOrdinal ? { backgroundColor: `${ORDINAL_COLOR}0A` } : {}}
                        >
                          ...
                          <td className="px-4 py-3">
                            {ct && (
                              <div className="flex items-center gap-1.5">
                                ...
                                />
                                <span className="text-sm text-slate-600">{ct.name}</span>
                              </div>
                            )}
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-sm text-slate-600">{priorityLabels[item.priority ?? 'medium']}</span>
                            </div>
                          </td>
                        </tr>
                      );
