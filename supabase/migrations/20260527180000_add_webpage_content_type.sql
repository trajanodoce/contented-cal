-- Add "Webpage" content type to all existing workspaces
INSERT INTO public.content_types (workspace_id, name, icon, color)
SELECT w.id, 'Webpage', 'Globe', '#0EA5E9'
FROM public.workspaces w
WHERE NOT EXISTS (
  SELECT 1 FROM public.content_types ct
  WHERE ct.workspace_id = w.id AND ct.name = 'Webpage'
);

-- Update seed function to include Webpage for new workspaces
