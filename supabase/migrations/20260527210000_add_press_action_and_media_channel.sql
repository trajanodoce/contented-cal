-- Add "Press Action" content type to all existing workspaces
INSERT INTO public.content_types (workspace_id, name, icon, color)
SELECT w.id, 'Press Action', 'Zap', '#D946EF'
FROM public.workspaces w
WHERE NOT EXISTS (
  SELECT 1 FROM public.content_types ct
  WHERE ct.workspace_id = w.id AND ct.name = 'Press Action'
);

-- Update channel values on intake forms to include Media/External
UPDATE public.intake_form_fields
SET options = options || '["Media/External"]'::jsonb
WHERE field_key = 'channel'
  AND field_type = 'single_select'
  AND NOT (options @> '"Media/External"');

-- Update seed function to include Press Action for new workspaces
...
BEGIN
  INSERT INTO public.content_types (workspace_id, name, icon, color) VALUES
    ...
    (NEW.id, 'Press Action', 'Zap', '#D946EF'),
    (NEW.id, 'Webpage', 'Globe', '#0EA5E9');
