-- Information disclosure fix: get_workspace_stats was callable via RPC by
-- any authenticated user with any workspace_id, returning item counts,
-- upcoming due item titles, and the last 15 activity log entries
-- (including usernames) for that workspace. Gate on workspace membership.
--
-- For legitimate callers the change is invisible (HomePage only passes
-- currentWorkspace.id, where the user is already a member). For attackers
-- guessing/enumerating workspace_ids, the function now returns NULL.
-- HomePage already handles a NULL response gracefully (see
-- src/pages/HomePage.tsx:76 — `if (statsResult.data) setStats(...)`).
